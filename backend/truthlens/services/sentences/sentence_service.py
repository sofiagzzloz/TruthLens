...
def _extract_sentence_slices(text: str) -> List[SentenceSlice]:
	...
	for match in _SENTENCE_PATTERN.finditer(text):
		...
		slices.append(SentenceSlice(content=stripped, start_index=start, end_index=end))
...
def sync_document_sentences(*, document: Document, text: Optional[str] = None) -> List[Sentence]:
	...
	existing = list(
		Sentence.objects.filter(document_id=document).order_by("start_index", "sentence_id")
	)
	...
	with transaction.atomic():
		if to_delete:
			Sentence.objects.filter(pk__in=[sentence.pk for sentence in to_delete]).delete()
